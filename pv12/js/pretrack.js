...
var initialSrc = "https://lp.dailytipsupdate.com/"+campaignID+"?format=json";
...
setTimeout(function(){
	if (!urlParams.get('rtkcid')) {
		...
		xhr.onreadystatechange = function() {
			if (xhr.readyState == 4 && xhr.status == 200) {
				...
				document.querySelectorAll('a').forEach(function(el) {
					if (el.href.indexOf("lp.dailytipsupdate.com/preclick")>-1) {
				...
				xhrr.open("GET", "https://lp.dailytipsupdate.com/preview?clickid="+rawData.clickid)
				xhrr.send();
			}
		}
		...
		xhrTrack.open("GET", "https://lp.dailytipsupdate.com/preview?clickid="+rtkClickID )
		xhrTrack.send();
		document.querySelectorAll('a').forEach(function(el) {
			if (el.href.indexOf("lp.dailytipsupdate.com/preclick")>-1) {
				if (el.href.indexOf('?')>-1) {
